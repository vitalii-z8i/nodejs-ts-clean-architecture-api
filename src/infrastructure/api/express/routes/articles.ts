...
import { ArticleFeed, CreateArticle, DeleteArticle, UpdateArticle, ViewArticle } from '../../../../use-cases/article'
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await controller.create({ token, body: { title, description, content, isPublished }})
    res.send(result)
  } catch (err) {
    return next(err)
  }
...
router.put('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const token = [...(req.headers['authorization']?.split(' ') || [])].pop() || ''
    const { title, description, content, isPublished } = req.body
    const result = await controller.update({ token, params: { id }, body: { title, description, content, isPublished }})
    res.send(result)
  } catch (err) {
    return next(err)
  }
